...
export function toDocument(row: DocumentRow) {
  ...
  const categoryRow = (Array.isArray(row.categories) ? row.categories[0] : row.categories) as CategoryRow;
  const category = categoryRow ? toCategory({ ...categoryRow, position: categoryRow.position ?? 0 }) : null;
