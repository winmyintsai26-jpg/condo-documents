...
import { DOCUMENT_SELECT, storagePath, toDocument, validatePdfBytes } from "./_shared/documents";
...
export default async (request: Request) => {
  ...
  const { data: old, error: lookupError } = await db.from("documents").select("storage_path").eq("id", id).single();
  if (lookupError) return json({ message: "Document not found." }, 404);
  const path = storagePath(file.name); const uploaded = await db.storage.from(DOCUMENT_BUCKET).upload(path, await bytesPromise, { contentType: "application/pdf" });
  if (uploaded.error) return json({ message: "The PDF could not be uploaded." }, 500);
  const { data, error } = await db.from("documents").update({ storage_path: path, file_name: file.name, file_size: file.size, file_type: "PDF" }).eq("id", id).select(DOCUMENT_SELECT).single();
