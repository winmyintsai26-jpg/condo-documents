...
const configuredUsername = (process.env.ADMIN_USERNAME ?? "").trim(); const configuredHash = (process.env.ADMIN_PASSWORD_HASH ?? "").trim();
...
const username = (await prompt("Enter configured admin username: ")).trim(); const password = await prompt("Enter original admin password: ", true);
