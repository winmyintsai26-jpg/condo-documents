...
async function api<T>(url: string, init?: RequestInit): Promise<T> { const response = await fetch(url, { credentials: "same-origin", ...init }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.message ?? "The request could not be completed."); return body as T; }
...
export const getAdminCategories = () => api<{categories: CategoryDefinition[]}>("/api/admin/categories");
